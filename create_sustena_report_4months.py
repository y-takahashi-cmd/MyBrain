...
members = [
    ...
    },
    {
        ...
        'summary': (
            ...
            '【セッション概要】\n'
            '4月：社内複数名から高い信頼を寄せられているという客観的な声をフィードバック。睡眠が改善傾向。'
            '6月：「報連相が届かない」という組織課題を言語化し、「顧客視点で考える」という判断軸を自ら決断。'
        ),
    },
    {
        'name': '大森 美葉さん',
    ...
    },
    {
        ...
        'summary': (
            '【見立て】\n'
            '職務意欲・目的意識は良好であり、経営者としての強みとなっている。一方で身体的指標に複数の要注意項目が確認されており、健康診断の結果と合わせて医療機関受診のご検討をお勧めしたい状態にある。\n\n'
            '【セッション概要】\n'
            '問診票・プロファイリングの書面分析を実施。消化器系・ホルモンバランス・代謝機能等において要注意ライン超えを確認。'
            '対面面談は未実施のため、引き続き状況を確認しながらサポートいたします。'
        ),
    },
]
...
teigen = doc.add_paragraph(
    ...
    '　各セッションを通じて感じたこととして、評価基準や報連相に関するマニュアルの見直し時期にきているかもしれないという印象を受けました。ご参考まで申し添えます。\n\n'
...
r4 = h4.add_run('【次期継続に向けたご提案】')
r4.font.bold = True
r4.font.size = Pt(10)

teian = doc.add_paragraph(
    '　現在、50人以上の企業に義務化されているストレスチェックは、2025年5月の労働安全衛生法改正により2028年4月から中小企業にも拡大されます。次期からストレスチェックを導入し、高リスクと判定された方への継続的なフォロー窓口として保健室を位置づけることで、高リスクから中リスクへの改善を組織的に実現できます。あわせて、2025年4月に施行された改正育児・介護休業法により企業への設置が義務化された介護相談窓口としても一体的に担うことで、法令対応と従業員支援を同時に実現できる制度として確立できると考えます。\n\n'
    '　次期のご継続にあたり、これらの観点も含めてご検討いただけますと幸いです。'
)
teian.runs[0].font.size = Pt(10)
